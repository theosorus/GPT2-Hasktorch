...
import Data.File (loadJSON, readFileToPairs)
...
tokenMain ::  BS.ByteString -> FilePath -> FilePath -> [Int]
tokenMain input vocabPath pairsPath =
    ...
    let validArrayString = makeStrArray (replaceSpace tokenizer)  -- Makes a valid array of String as ["h","e","l","l","o","Ġ","!"]
        merged = tokenizer validArrayString -- Merged following GPT2 merges.txt
    in changeToIndex merged
